 ...
 * sensor of their device.
 * <p/>
 * Construct an instance of {@link RxFingerprint} by building it via the
 * {@link RxFingerprint.Builder}.
 * <p/>
 * To simply authenticate the user with their fingerprint, use {@link #authenticate()}.
 * <p/>
 * To encrypt given data and authenticate the user with their fingerprint,
 * call {@link #encrypt(String, String)}
 * <p/>
 * To decrypt previously encrypted data via the {@link #encrypt(String, String)}
...
public class RxFingerprint {
	...
	private RxFingerprint(Context context, boolean keyInvalidatedByBiometricEnrollment, EncryptionMethod encryptionMethod, RxFingerprintLogger logger) {
	...
	/**
	 * Authenticate the user with their fingerprint. This will enable the fingerprint sensor on the
	 * device and wait for the user to touch the sensor with their finger.
	 * <p/>
	...
	/**
	 * Encrypt data and authenticate the user with their fingerprint. The encrypted data can only be
	...
	/**
	 * Encrypt data  and authenticate the user with their fingerprint. The encrypted data can only be
	 * accessed again by calling {@link #decrypt(String, String)} with the same keyName.
	 * Encrypted data is only accessible after the user has authenticated with
	 * fingerprint authentication.
	 * <p/>
	 * Encryption uses AES encryption with CBC blocksize and PKCS7 padding.
	 ...
	 *                  {@link #encrypt(String)}.  @return Observable
	...
	/**
	 * Decrypt data previously encrypted with {@link #encrypt(String, String)}.
	 * To decrypt, you have to provide the same keyName that you used for encryption.
	 * <p/>
	 * The encrypted string should be exactly the one you previously received as a result of the
	 * {@link #encrypt(String, String)} method.
	 * <p/>
	 * The resulting {@link FingerprintDecryptionResult} will contain the decrypted string as a
	 * String and is accessible via {@link FingerprintDecryptionResult#getDecrypted()} if the
	 * authentication and decryption was successful.
	 ...
	 * successful or have failed entirely.
	 */
	public Observable<FingerprintDecryptionResult> decrypt(@Nullable String keyName, @NonNull String encrypted) {
		switch (encryptionMethod) {
			case AES:
				return AesDecryptionObservable.create(context, keyName, encrypted, keyInvalidatedByBiometricEnrollment, logger);
			case RSA:
				return RsaDecryptionObservable.create(context, keyName, encrypted, keyInvalidatedByBiometricEnrollment, logger);
			default:
				return Observable.error(new IllegalArgumentException("Unknown encryption method: " + encryptionMethod));
		}
	}
	...
	public boolean isHardwareDetected(@NonNull Context context) {
	 ...
	 * <p/>
	 * This would mean that the user has disabled the lock screen on their device or changed the
